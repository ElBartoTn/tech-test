...
class ProductFilter extends React.Component {
    ...
    */
    getItemsWithCount(filterType) {//get products based on filter type with count for each choice 
        let possibleItems;
        ...
        for(let i = 0; i < possibleItems.length; i++) {
            if(filterType === "product-type")
            itemsWithCount[possibleItems[i]] = products.filter((obj) => obj.type === possibleItems[i]).length;
            if(filterType === "product-brand")
            itemsWithCount[possibleItems[i]] = products.filter((obj) => obj.brand === possibleItems[i]).length;
        }
        return itemsWithCount;
       }
     getSelectedFilters() {//get selected filters search text and add it to a list
        let selectedFilters;
        const filters = this.props.selectedFilters;
        if(filters){
            selectedFilters = filters.map(filter => filter.searchText.map(s => s));
     ...
     isAnyFilterSelected() {
        ...
        if(filters){
            ...
            switch(filterBy){
                case "product-type" :
                    selectedFiltersByType = this.getSelectedfiltersByType("product-type", filters);
                    break;
                    case "product-brand" :
                    selectedFiltersByType = this.getSelectedfiltersByType("product-brand", filters);
                    break;
                    default:
                    return false;
            }
       ...
       getSelectedfiltersByType(filtertype, filters){
        let selectedFiltersByType = filters.map(filter => filter.filterBy === filtertype ? filter.searchText : []);
    ...
    render() {
        ...
        return (
            <Dropdown header={this.state.header} 
